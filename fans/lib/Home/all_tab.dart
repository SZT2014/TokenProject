...
class AllTab extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    List<WalletToken> allTokens = ref.watch(allTokensRequestProvider).valueOrNull ?? [];
    var tokens = useMemoized(() {
      ...
      if(sortType.value == TokenSortType.tokenSortDefault) {
        ...
        tokensList.sort(TokenSortType.symbolAsc.sortKey);
        return highestList + tokensList;
      } else {
        filterTokens.sort(sortType.value!.sortKey);
        return filterTokens;
      }
    }, [allTokens, sortType.value, content.value]);
